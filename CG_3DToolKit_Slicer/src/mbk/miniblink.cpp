...
void miniblink::forword()
{
   wkeGoForward(webView);
}
...
float miniblink::getZoom()
{
    return wkeGetZoomFactor(webView);
}

void miniblink::destroy()
{
    wkeDestroyWebView(webView);
}

QString miniblink::getURL()
{
    return QString::fromUtf8(wkeGetURL(webView));
}

void miniblink::setZoom(float factor)
{
